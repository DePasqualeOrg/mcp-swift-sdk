...
final class TypeExtractor: SyntaxVisitor {
    ...
    init(filePath: String, source: String) {
        ...
        self.locationConverter = SourceLocationConverter(fileName: filePath, tree: Parser.parse(source: source))
...
func extractTypesFromDirectory(_ path: String) throws -> [TypeInfo] {
    ...
    for case let fileURL as URL in enumerator {
        ...
        do {
            types.append(contentsOf: try extractTypesFromFile(fileURL.path))
        } catch {
...
struct ExtractSwiftTypes: ParsableCommand {
    ...
    func run() throws {
        ...
        if isDirectory.boolValue {
            types = try extractTypesFromDirectory(path)
        } else {
            types = try extractTypesFromFile(path)
        }
        ...
        if let typeName = check {
            ...
            if json {
                ...
                if let found {
                    ...
                    result = ["exists": false, "name": typeName]
                }
